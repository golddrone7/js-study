...
alert(`총을 받았습니다. ${gamerList[starting%gamerList.length]}부터 시작합니다.`);
outer :for(var i=0; i<gun.length; i++){
    prompt(`[${gamerList[starting%gamerList.length]}님의 턴!] 탄창을 회전합니다.
    # 엔터를 누르면 격발합니다.`);
    //총알이 있을 때
    if(gun[i]===1){
        gun[i]=0;
        alert(`# 빵!! [${gamerList[starting%gamerList.length]}]님이 사망...RIP`);
        gamerList.splice(starting%gamerList.length,1);
    } else{
        alert(`...휴 ~~ 살았습니다.`);
        starting++;
    }
